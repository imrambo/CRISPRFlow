...
#------------------------------------------------------------------------------
def command_generator(seqdb, program='hmmsearch', profile, psuffix='.hmm', optdict, prefix*, parallel=False, jobs*, joblog*, progbar=False):
    """
    Generate a list of command strings.
    Use parallel=True to run parallel jobs.
    """
    if not prefix:
        prefix = get_basename(seqdb)
    commands = []
    ...
    if not parallel:
        ...
        if isinstance(profile, list):
            ...
            if len(pfiles) + len(pdirs) == len(profile):
                    if pdirs:
                        for pdir in pdirs:
                            cmd = 'find %s -type f -name "*%s" | parallel %s hmmsearch --domtbl %s/{/.}_%s.domtbl %s {} %s' % (pdir, psuffix, parallel_optstring, outdir, prefix, hmmsearch_optstring, seqdb)
                            commands.append(cmd)
                    if pfiles and len(pfiles) > 1:
                        cmd = 'parallel %s hmmsearch --domtbl %s/{/.}_%s.domtbl %s {} %s ::: %s' % (pdir, psuffix, parallel_optstring, outdir, prefix, hmmsearch_optstring, seqdb, ' '.join(pfiles))
                        commands.append(cmd)
